...
def _gini_like(seq: str) -> float:
    ...
    counts = Counter(seq)
    freqs = np.array(
        sorted(counts.get(aa, 0) / n for aa in _AA20_LIST), dtype=float
    )
    total = freqs.sum()
    if total == 0:
        return _NAN
...
@register("complexity", family="complexity")
class ComplexityDescriptor(BaseDescriptor):
    ...
    Output columns (prefix ``complexity_``):
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize ComplexityDescriptor."""
    ...
    def _nan_schema(self, feats: dict[str, float]) -> dict[str, float]:
        ...
        for key in (
            "shannon_entropy", "shannon_entropy_norm",
            "linguistic_complexity_k1", "linguistic_complexity_k2", "linguistic_complexity_k3",
            "unique_kmer_fraction_k2", "unique_kmer_fraction_k3",
            "repeated_kmer_fraction_k2", "repeated_kmer_fraction_k3",
            "homopolymer_burden_len2", "homopolymer_burden_len3",
            f"low_complexity_window_fraction_w{w}",
            f"window_entropy_mean_w{w}", f"window_entropy_std_w{w}",
            "most_frequent_residue_fraction",
            "gini_like_inequality", "residue_dominance_gap",
        ):
            feats[key] = _NAN
    ...
    def compute_one(self, sequence: str) -> dict[str, float]:
        ...
        for k in (2, 3):
            ...
            if not words:
                ...
                feats[f"repeated_kmer_fraction_k{k}"] = (
                    sum(v for v in counts.values() if v > 1) / len(words)
                )
        ...
        feats["residue_dominance_gap"] = (
            (most_common[0][1] - most_common[1][1]) / n
