"""Shared low-level utilities for sequence descriptor computation.

This module hosts low-risk scalar helpers reused across descriptor
families. Canonical preprocessing policy now lives in
``roxy.sequence.preprocessing.cleaning`` and
``roxy.sequence.preprocessing.validation``. These helpers are primarily
for internal family implementations rather than the high-level public API.
"""
