"""Internal base abstractions for sequence descriptor components.
...
- an optional bridge to the active DataFrame-oriented engine interface.

Only the sequence-block interfaces are intended to be broadly useful.
Lower-level engine abstractions remain available by explicit module
import from ``roxy.sequence.base.engine`` when needed.
"""
...
from .engine import BaseDescriptorEngine
